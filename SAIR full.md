...
A magma is a set $M$ with a binary operation $* : M \times M \to M$. In mathematics binary operations are ubiquitous, the most common being addition and multiplication. And so examples of magmas are the real numbers equipped with the binary operation of addition, or the real numbers equipped with the binary operation of multiplication. More generally, any group is a magma. The conditions placed on the binary operation of a group are very restrictive. On the other hand, the binary operation of a generic magma only satisfies that $a * b$ is again an element of the magma. The word magma evokes an amorphous object and that is the right way to think about this. While there is only one group with three elements up to isomorphism, there are 3330 magmas with 3 elements up to isomorphism. For four elements, there are two groups up to isomorphism and 178981952 magmas up to isomorphism.
...
One way to impose an extra condition is by means of an equation, such as
...
The latter equation implies that the magma contains only one element. On the other hand, the equation
...
The cleaned-up code for the analysis is available at [github.com/lschiemanowski/SAIR_stage_1](https://github.com/lschiemanowski/SAIR_stage_1).
...
![PCA projection colored by the learned 8 clusters](blogpost_cluster_pca_learned8.svg)
...
![The same PCA projection colored by the 4-type coarsening](blogpost_cluster_pca_coarsened4.svg)
...
![The same PCA projection colored by the syntax rules](blogpost_cluster_pca_syntax4.svg)
...
| 5 | Special Weak Type | Usually left-hand side $x*y$; implies type 4 and type 5 |
